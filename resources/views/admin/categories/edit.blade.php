<x-html>
    ...
    <section class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 p-4 -mt-32">
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
            <h2 class="mb-4 text-xl font-bold text-gray-900">Edit Kategori: {{ $category->name }}</h2>
            ...
            <form action="{{ route('admin.categories.update', $category) }}" method="POST">
                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" id="name" 
                               ...
                               class="{{ $inputClass }}" placeholder="Masukkan nama kategori" required>
                        @error('name')
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <div class="mb-2">
                            <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold text-white {{ old('color', $category->color) }}">Preview: {{ old('color', $category->color) }}</span>
                        </div>
                        <input type="text" name="color" id="color" 
                               value="{{ old('color', $category->color) }}"
                               class="{{ $inputClass }}" placeholder="Contoh: bg-red-500" required>
                        <p class="mt-2 text-xs text-gray-500">Gunakan format kelas Tailwind CSS (e.g., bg-blue-600).</p>
                        @error('color')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex items-center space-x-4 mt-6">
                    <button type="submit" 
                            class="inline-flex items-center px-5 py-2.5 text-sm font-medium border border-gray-400 text-black bg-green-600 rounded-lg hover:bg-green-700">
                        Update Kategori
                    </button>
                </div>
            </form>
